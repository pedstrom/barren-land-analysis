...
Out of memory! Looks like I ran out of memory on this computer after just a percent or two of the problem space. 
...
Looks like it found the first area successfully! It's area is below. One thing worth noting is that on this Mid 2011 iMac, this took close to 2 hours to complete. Clearly optimization would be helpful here. I have some ideas on where there are inefficiencies, but let's validate the output and move on to finding all areas first. 
...
This is a match to the area shown in the Sample Output. 
...
Whoops. Think I used the wrong data frame here. Needed to put it all into a loop anyway, so here goes:
...
while (!is.na(nextFertile <- dfFlood[dfFlood$z==1,][1,])) {
  floodFill(nextFertile$x,nextFertile$y) # prime the Q and next area
